...
server <- function(input, output) {
  ...
  output$recessionPlot <- renderPlotly ({
    ...
    title <- paste0("Stock Market Dataset:", " Time v.s. ", 
                    input$y_recession_var)

    # Recession plots
    if (input$company_data == "Amazon") {
      # Create amazon ggplot
      amazon_plot <- ggplot(recession_amazon) +
        geom_point(mapping = aes_string(x = recession_amazon$Date, 
                                        y = input$y_recession_var),
      ...
      apple_plot <- ggplot(recession_apple) +
        geom_point(mapping = aes_string(x = recession_apple$Date, 
                                        y = input$y_recession_var),
                   size = input$recession_size,
                   color = "blue") +
        aes(text = paste("Company:", Company)) +
      ...
      microsoft_plot <- ggplot(recession_msoft) +
        geom_point(mapping = aes_string(x = recession_msoft$Date, 
                                        y = input$y_recession_var),
                   size = input$recession_size,
                   color = "red") +
        aes(text = paste("Company:", Company)) +
      ...
      all_plot <- ggplot() +
        geom_point(recession_amazon, mapping = aes_string(
          x = recession_amazon$Date, y = input$y_recession_var),
                   size = input$recession_size,
                   color = "orange") +
        geom_point(recession_apple, mapping = aes_string(
          x = recession_apple$Date, y = input$y_recession_var),
                   size = input$recession_size,
                   color = "blue") +
        geom_point(recession_msoft, mapping = aes_string(
          x = recession_msoft$Date, y = input$y_recession_var),
                   size = input$recession_size,
                   color = "red") +
        aes(text = paste("Company:", Company)) +
        labs(x = "Year", y = input$y_recession_var, title = title)
  ...
  output$mainTable <- DT::renderDataTable({covidPlotTable}, 
                                          selection = 'single', 
  ...
  output$covidPlot <- renderPlot({
    ...
    if(input$lockdowns){
      lockdownDates <- combined %>% filter(Date > as.Date('2020-3-19')) %>% 
        filter(Date < as.Date('2020-5-4'))
      covidPlot <- covidPlot + geom_point(data = lockdownDates, 
                                          aes(x = cases, y = Close), 
                                          size = 3, color = '#54ffeb')
    }
    ...
    if(length(selectedRows)){
      ...
      covidPlot <- covidPlot + 
        geom_point(data = toPlot, aes(x = cases, y = Close), 
                   color = "#ab82d9", size = 4) +
        geom_vline(aes(xintercept = combined[selectedRows[[1]], 2]), 
                   color = "#ab82d9", size = 2)
  ...
  output$electionplot <- renderPlotly({
    ...
    if(input$company_data2 == "Amazon"){
      amzn_election_plot <- ggplot(election_amazon) +
        geom_point(mapping = aes_string(x = election_amazon$Date, 
                                        y = input$y_election_var),
     ...
     ggplotly(amzn_election_plot)
       ...
       apple_election_plot <- ggplot(election_apple) +
         geom_point(mapping = aes_string(x = election_apple$Date, 
                                         y = input$y_election_var),
                     size = input$election_size,
                    color = "blue") +
         aes(text = paste("Company:", Company)) +
       ...
       msoft_election_plot <- ggplot(election_msoft) +
         geom_point(mapping = aes_string(x = election_msoft$Date, 
                                         y = input$y_election_var),
                    size = input$election_size,
                    color = "red") +
         aes(text = paste("Company:", Company)) +
